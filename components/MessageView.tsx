...
import { getAssistantMessageStatusText } from "@/lib/assistant-status";
import type {
  AgentMessage,
  UserMessage,
  AssistantMessage,
...
function AssistantMessageView({
  ...
  const statusText = getAssistantMessageStatusText(message);
  const [hovered, setHovered] = useState(false);
  const [copied, setCopied] = useState(false);
  ...
  if (blocks.length === 0 && !isStreaming && !statusText) return null;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {statusText && (
          <div style={{ color: "#f87171", fontSize: 14, lineHeight: 1.6, whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>
            {statusText}
          </div>
        )}
      </div>

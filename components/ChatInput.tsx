...
import { getNextSendShortcut, shouldSendOnEnter, useSendShortcut } from "@/hooks/useSendShortcut";
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (e.key === "Enter" && shouldSendOnEnter(sendShortcut, e.metaKey || e.ctrlKey, e.shiftKey)) {
        e.preventDefault();
    ...
    [isStreaming, onSteer, onFollowUp, onAbort, slashMenuOpen, slashQuery, filteredSlashCommands, slashActiveIndex, applySlashCommand, sendQueued, handleSend, getNextSlashIndex, atMenuOpen, atQuery, atMatches, atActiveIndex, applyAtCompletion, sendShortcut]
  );
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div ref={controlsMenuRef} style={{
            ...
            <button
              type="button"
              onClick={() => setSendShortcut(getNextSendShortcut(sendShortcut))}
              title={sendShortcut === "mod-enter-send" ? "Send with Cmd/Ctrl+Enter; click to send with Enter" : "Send with Enter; click to send with Cmd/Ctrl+Enter"}
              aria-label={sendShortcut === "mod-enter-send" ? "Send shortcut: Cmd or Ctrl plus Enter" : "Send shortcut: Enter"}
              style={{
                display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                height: 32,
                padding: "0 8px",
            ...
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <path d="M7 9h.01M11 9h.01M15 9h.01M7 13h.01M11 13h6M7 17h10" />
              </svg>
              {(!isMobile || controlsMenuOpen) && (
                <span style={{ whiteSpace: "nowrap" }}>
                  {sendShortcut === "mod-enter-send" ? "Cmd/Ctrl+Enter" : "Enter"}
                </span>
              )}
            </button>

...
test("keeps an empty assistant error as the final displayed message", async () => {
  const { splitFinalAssistantMessage } = await loadSubject();
  const message = {
    ...assistant([]),
    stopReason: "error",
    errorMessage: "Invalid reasoning effort",
  };

  const result = splitFinalAssistantMessage(message);

  assert.equal(result.processMessage, null);
  assert.deepEqual(result.answerMessage?.content, []);
  assert.equal(result.answerMessage?.stopReason, "error");
  assert.equal(result.answerMessage?.errorMessage, "Invalid reasoning effort");
});

test("renders an assistant error only on the final message split", async () => {
  const { splitFinalAssistantMessage } = await loadSubject();
  const message = {
    ...assistant([
      { type: "thinking", thinking: "work through it" },
  ...
  assert.deepEqual(result.processMessage?.content.map((block) => block.type), ["thinking"]);
  assert.equal(result.processMessage?.stopReason, undefined);
  assert.equal(result.processMessage?.errorMessage, undefined);
  assert.equal(result.processMessage?.usage, undefined);
  assert.deepEqual(result.answerMessage?.content.map((block) => block.type), ["text"]);
  assert.equal(result.answerMessage?.stopReason, "error");
  assert.equal(result.answerMessage?.errorMessage, "Connection closed");
});

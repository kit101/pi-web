...
test("file manager commands use platform-native reveal behavior", () => {
  assert.deepEqual(
    buildFileManagerCommand({ path: "/work/a b.ts", isDirectory: false }, "darwin"),
    { command: "open", args: ["-R", "/work/a b.ts"] },
  );
  assert.deepEqual(
    buildFileManagerCommand({ path: "C:\\work\\a b.ts", isDirectory: false }, "win32"),
    { command: "explorer.exe", args: ["/select,C:\\work\\a b.ts"] },
  );
  assert.deepEqual(
    buildFileManagerCommand({ path: "/work/project", isDirectory: true }, "linux"),
    { command: "xdg-open", args: ["/work/project"] },
  );
});
...
test("file manager targets accept allowed files and directories", () => {
  assert.deepEqual(
    resolveAllowedFileManagerTarget(realFile, new Set([allowedRoot])),
    { path: realpathSync(realFile), isDirectory: false },
  );
  assert.deepEqual(
    resolveAllowedFileManagerTarget(canonicalDirectory, new Set([allowedRoot])),
    { path: realpathSync(canonicalDirectory), isDirectory: true },
  );
});

test("file manager targets cannot escape an allowed root", () => {
  assert.throws(
    () => resolveAllowedFileManagerTarget(pathThroughEscapingSymlink, new Set([allowedRoot])),
    /Access denied/,
  );
});
